...
2. constructor inference: a constructor parameter whose type inherits from `ObservableObject`
...
The `ViewModel` setter is also the initialization trigger: assigning a non-null value calls `InitializeComponent()`.
...
- Initialization is currently assignment-driven rather than idempotent, so repeated non-null `ViewModel` assignment reruns `InitializeComponent()`.
