...
public partial class NodeHealthCard : FrameView
{
    public Label NodeTitle { get; private set; } =
        new() { BindText = nameof(ClusterExplorerViewModel.NodeName) };

    public Label NodeIpLabel { get; private set; } =
        new() { Below = nameof(NodeTitle), BindText = nameof(ClusterExplorerViewModel.NodeIp) };

    // Level 4 Subview: CPU Resource Meter
    public ResourceMeterWidget CpuMeter { get; private set; } =
        new()
        {
    ...
    public RamMeterWidget RamMeter { get; private set; } =
        new()
        {
